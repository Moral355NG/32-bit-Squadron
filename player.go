...
func (p *Player) Update() {
	...
	if ebiten.IsKeyPressed(ebiten.KeyArrowLeft) && p.x > 0+p.width/2 && gameOver == false {
		p.x -= p.vel
		p.state = left
	} else if ebiten.IsKeyPressed(ebiten.KeyArrowRight) && p.x < 1280-p.width/2 && gameOver == false {
		p.x += p.vel
		p.state = right
	} else {
		p.state = centre
